$(function(){
	...
	$("#add-form").form({
		...
		success : function(data){
			...
			if ( result ){
				$.messager.alert('提示','教学论文信息新增成功！','info') ;
				$("#add-box").dialog('close') ;
				$("#add-form").form('reset') ;
				$("#data-box").datagrid('reload') ;
			} else {
				$.messager.alert('提示','教学论文信息新增失败！','info') ;
			}
		}
	});
	...
	$("#edit-form").form({
		...
		success : function(data){
			...
			if ( result ){
				...
				$("#data-box").datagrid('reload') ;
				$.messager.alert('提示','教学论文信息更新成功！','info') ;
			} else {
				$.messager.alert('提示','教学论文信息更新失败！','info') ;
			}
		}
	});
...
function detail(id){
	$.ajax({
					...
					success : function(data){
						...
						if ( data.file_name ){
							$("#detail-file_name").text('点击查看') ;
							$("#detail-file_name").attr('onclick','window.open("' + ROOT + '/Uploads/Teach_Paper/' + data.file_name + '")') ;
						} else {
							$("#detail-file_name").text('暂无附件').removeAttr('onclick') ;
						}
					}
...
function remove(id){
	$.messager.confirm('删除提示','您确定要删除这条教学信息吗？',function(r){
					if ( r ){
						$.ajax({
						url : APP + '/Teach/Paper/delete' ,
						method : 'post' ,
						...
						success : function(data){
							if ( data ){
								$("#data-box").datagrid('reload') ;
								$.messager.alert('提示','教学论文信息删除成功！','info') ;
							} else {
								$.messager.alert('提示','教学论文信息删除失败！','info') ;
							}
						}
					});
					}
