...
class PatentController extends CommonController{
    ...
    public function exportSelect(){
        ...
        for ( $i = 0 ; $i < count($selects) ; $i ++ ){
            $item = $patent -> where('id='. $selects[$i]) -> find() ;
        ...
        for ( $i = 2 ; $i < count($data) + 2 ; $i ++ ){
            $objPHPExcel->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('D'.$i, $data[$i-2]['category'])
                    ->setCellValue('E'.$i, $data[$i-2]['state'])
                    ->setCellValue('F'.$i, $data[$i-2]['unit_name'])
                    ->setCellValue('G'.$i, $data[$i-2]['application_id'])
                    ->setCellValue('H'.$i, $data[$i-2]['application_date'])
                    ->setCellValue('I'.$i, $data[$i-2]['auth_id'])
                    ->setCellValue('J'.$i, $data[$i-2]['auth_date'])
    ...
    public function downloadSelect(){
        ...
        //带打包的附件集合
        for ( $i = 0 ; $i < count($selects) ; $i ++ ){
            $item = $patent -> where('id='. $selects[$i]) -> getField('file_name') ;
            if ( !empty($item) )
            {
                $files[] = zh_cn('./Uploads/Patent/' . $item ) ;
            }
        }
    ...
    public function downloadAll (){
        ...
        $rows = $patent -> where($search) -> field('file_name') -> select () ;
