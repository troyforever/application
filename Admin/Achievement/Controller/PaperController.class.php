...
class PaperController extends CommonController{
    ...
    public function exportSelect(){
        ...
        for ( $i = 0 ; $i < count($selects) ; $i ++ ){
            $item = $paper -> where('id='. $selects[$i]) -> find() ;
    ...
    public function exportAll(){
        ...
        for ( $i = 2 ; $i < count($rows) + 2 ; $i ++ ){
            $objPHPExcel->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('B'.$i ,$rows[$i-2]['abstract'])
                    ->setCellValue('C'.$i, $rows[$i-2]['keywords'])
                    ->setCellValue('D'.$i, $rows[$i-2]['first_author'])
                    ->setCellValue('E'.$i, $rows[$i-2]['other_author'])
                    ->setCellValue('F'.$i, $rows[$i-2]['unit_name'])
                    ->setCellValue('G'.$i, $rows[$i-2]['publication'])
                    ->setCellValue('H'.$i, $rows[$i-2]['publication_date'])
                    ->setCellValue('I'.$i, $rows[$i-2]['final_index'])
                    ->setCellValue('J'.$i, $rows[$i-2]['sci_partition'])
                    ->setCellValue('K'.$i, $rows[$i-2]['index_date'])
                    ->setCellValue('L'.$i, $rows[$i-2]['if_num'])
                    ->setCellValue('M'.$i, $rows[$i-2]['note'])
                    ->setCellValue('N'.$i, $rows[$i-2]['file_name']) ;
        }
        // Rename worksheet
        $objPHPExcel->getActiveSheet()->setTitle('Sheet1');
        // Set active sheet index to the first sheet, so Excel opens this as the first sheet
    ...
    public function downloadSelect(){
        ...
        //带打包的附件集合
        for ( $i = 0 ; $i < count($selects) ; $i ++ ){
            $item = $paper -> where('id='. $selects[$i]) -> getField('file_name') ;
            if ( !empty($item) )
            {
                $files[] = zh_cn('./Uploads/Paper/' . $item ) ;
            }
        }
    ...
    public function downloadAll (){
        ...
        $rows = $paper -> where($search) -> field('file_name') -> select () ;
