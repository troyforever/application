...
class PaperController extends CommonController{
    ...
    public function exportSelect(){
        ...
        for ( $i = 2 ; $i < count($data) + 2 ; $i ++ ){
            $objPHPExcel->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('B'.$i, $data[$i-2]['first_author'])
                    ...
                    ->setCellValue('G'.$i, $data[$i-2]['final_index'])
                    ->setCellValue('H'.$i, $data[$i-2]['sci_partition'])
                    ->setCellValue('I'.$i, $data[$i-2]['index_date'])
                    ->setCellValue('J'.$i, $data[$i-2]['if_num'])
    ...
    public function downloadSelect(){
        ...
        for ( $i = 0 ; $i < count($selects) ; $i ++ ){
            ...
            if ( !empty($item) )
            {
                $files[] = zh_cn('./Uploads/Teach_Paper/' . $item ) ;
            }
        }
